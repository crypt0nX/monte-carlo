...
for N = totalPointCounts
    ...
    f3 = ezplot('(x-2)*(x-2)+(y-2)*(y-2)=4', [-1, 4, -1, 4]);
    set(f3, 'LineWidth', 4, 'color', 'k');
    ...
    for i = 1:N
        xpoints = points(i, 1);
        ypoints = points(i, 2);
        if ((xpoints*(3/2)+(1/2) > ypoints) && (ypoints > xpoints*(3/8)-(5/8))  && (xpoints*(-3/4)+(11/4) > ypoints) && (xpoints-2)*(xpoints-2)+(ypoints-2)*(ypoints-2)<4)
